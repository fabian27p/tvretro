...
class ChannelLibrary:
    ...
    def name(self,n):
        ...
        return 'SIN SEÑAL'
    def choose(self,n,mode,previous=None,avoid_repeat=True,resume=None):
        vs=self.videos(n)
        if not vs: return None
        if mode=='resume' and resume:
            p=Path(str(resume.get('path','')))
            if p in vs: return p
        if mode=='daily':
            key=f'{n}:{date.today().isoformat()}'.encode()
            idx=int(hashlib.sha1(key).hexdigest(),16)%len(vs)
            return vs[idx]
        if mode=='sequential':
