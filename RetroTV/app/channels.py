...
class ChannelLibrary:
    ...
    def metadata(self,n):
        r=self.root(n)
        for name in ('channel.json','.channel.json'):
            p=r/name
            if p.exists():
    ...
    def videos(self,n):
        ...
        return sorted([p for p in r.rglob('*') if p.is_file() and not p.name.startswith('.') and p.name!='channel.json' and p.suffix.lower() in self.ext],key=lambda p:str(p).lower())
    def name(self,n):
